...
PACKAGE_NAME = "notifications_service_app"
...
from notifications_service_app import create_app  # type: ignore
from notifications_service_app.dependencies import DependencyBundle  # type: ignore
from notifications_service_app.event_reader import EventReader  # type: ignore
...
def token_factory() -> Callable[[Dict], str]:
    def _create(claims: Dict) -> str:
        settings = get_settings()
        payload = {"sub": claims.get("sub", "user-1"), "role": claims.get("role", "organizer")}
    ...
    def _create(
        *,
        ...
        event = Event(
            ...
            description="Event for notifications tests",
